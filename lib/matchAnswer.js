/**
 * Maps single-word player-specific nicknames to their full canonical name.
 * Checked first — if the entire guess matches a key, it is treated as if the
 * player had typed the full name instead.
 *
 * Rules: only unambiguous, widely-known NRL nicknames are included.
 * The value must exactly match how the player's name appears in the database.
 */
const PLAYER_NICKNAME_MAP = {
  joey: 'andrew johns',
  alfie: 'allan langer',
  locky: 'darren lockyer',
  freddie: 'brad fittler',
  sterlo: 'peter sterling',
  blocker: "mark o'meley",
  gus: 'phil gould',
  turbo: 'tom trbojevic',
  rts: 'roger tuivasa-sheck',
  choc: 'steve price',
  fatty: 'paul vautin',
  artie: 'arthur beetson',
  bozo: 'peter sterling',
  petero: 'petero civoniceva',
};
...
export function matchAnswer(guess, answer) {
  let cleanedGuess = cleanName(guess);
  const cleanedAnswer = cleanName(answer);

  // If the entire guess is a recognised player-specific nickname, resolve it to
  // the canonical full name before any further matching.
  if (PLAYER_NICKNAME_MAP[cleanedGuess]) {
    cleanedGuess = PLAYER_NICKNAME_MAP[cleanedGuess];
  }
