...
function buildHtml(gameNumber, subscriber) {
  const unsubUrl = subscriber.unsubscribe_token
    ? `${SITE_URL}/api/unsubscribe?token=${encodeURIComponent(subscriber.unsubscribe_token)}`
    : `${SITE_URL}/api/unsubscribe?email=${encodeURIComponent(subscriber.email)}`;
...
async function getSubscribers() {
  const withTokens = await supabaseAdmin
    .from('email_subscribers')
    .select('email, unsubscribe_token');

  if (!withTokens.error) return withTokens;
...
export async function sendDailyEmail(gameNumber) {
  ...
  for (let i = 0; i < subscribers.length; i += BATCH_SIZE) {
    ...
    const messages = chunk.map((subscriber) => ({
      ...
      to: subscriber.email,
      subject,
      html: buildHtml(gameNumber, subscriber),
    }));
